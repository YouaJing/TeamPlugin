    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (subcommand) {
            ...
            case "new" :
                ...
                int playTime = player.getStatistic(Statistic.PLAY_ONE_MINUTE) / 3600; // 获取玩家的总游玩时长，单位为小时
            ...
            case "invite":
                ...
                // 发送邀请消息给目标玩家
                player.playSound(target.getLocation(), Sound.UI_BUTTON_CLICK, 1.0f, 1.0f);
                target.sendMessage(ChatColor.GOLD + "你收到了来自" + player.getName() + "的团队" + MsgUtil.color("&" + team.getColor().replace("<", "").replace(">", "")) + team.getName() + ChatColor.GOLD + "的邀请，输入/team accept或/team reject来回应。");
            ...
            case "list":
                ...
                for (int i = (page - 1) * pageSize; i < page * pageSize && i < teams.size(); i++) {
                    ...
                    int size = t.getMembers().size() + 1; // 加一是为了包括队长
            ...
            case "members":
                ...
                // 发送团队成员的内容
                player.sendMessage(ChatColor.GRAY + "- " + members.toString());
            ...
            case "reject":
                ...
                // 发送通知消息给团队的队长
                leader = Bukkit.getPlayer(team.getLeader());
                fushou1 = Bukkit.getPlayer(team.getFushou());
                if (leader != null) {
                    leader.sendMessage(ChatColor.DARK_RED + player.getName() + "拒绝了你的团队邀请！");
                }
                if (fushou1 != null) {
                    fushou1.sendMessage(ChatColor.DARK_RED + player.getName() + "拒绝了你的团队邀请！");
                }
                return true;
            ...
            case "quit":
                ...
                fushou1 = Bukkit.getPlayer(team.getFushou());
                if (leader != null) {
                    leader.sendMessage(ChatColor.DARK_RED + "很遗憾" + player.getName() + "退出了你的团队！");
                }
                if (fushou1 != null) {
                    fushou1.sendMessage(ChatColor.DARK_RED + "很遗憾" + player.getName() + "退出了你的团队！");
                }
